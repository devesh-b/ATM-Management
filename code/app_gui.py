...
class MainUI(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        a=sql.connect( host="localhost",
                       user="root",
                       password="sai!@#2345",
                       database='ATM'
                      )
        sql_select_Query = "SELECT * FROM cust_details"
        cursor = a.cursor()
...
class StartPage(tk.Frame):
        ...
        def __init__(self, parent, controller):
            ...
            label = tk.Label(self, text="        PESU ATM        ", font=self.controller.title_font,fg="#263942")
            label.grid(row=0, sticky="ew")
            button1 = tk.Button(self, text="   SIGNUP  ", fg="#ffffff", bg="#263942",command=lambda: self.controller.show_frame("PageOne"))
...
class PageOne(tk.Frame):
    ...
    def start_training(self):
        ...
        def rand(self):
         ...
         s=0.0
         while multi>0:     
          s+=random.random()*multi
          multi/=10
         return s                               #returning random customer id and total amount
 
        random.seed(30)
        ids=int(rand(self))
        amount=((int)(rand(self)*random.random()*6.89))/2   #initial total amount of money present
        ...
        sq = "INSERT INTO ATM (acct_no,name,amt_savings,amt_checkings) VALUES (%s, %s, %s, %s)"
        val = (ids,name,amount,amount)
...
class PageFour(tk.Frame):
    ...
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.controller = controller
        label = tk.Label(self, text="Face Recognition", font='Helvetica 20 bold')
        ...
        button4.grid(row=1,column=3, sticky="ew", ipadx=5, ipady=4, padx=10, pady=10)
 
    def openwebcam(self):
        impname=main_app(self.controller.active_name)
        sub_app(impname)
