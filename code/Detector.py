...
def main_app(names):
        ...
        global name
        name=names
        s="./data/classifiers/"+name+"_classifier.xml"
        recognizer.read(s)
        ...
        while True:
            ...
            if cv2.waitKey(10) & 0xFF == ord('q'):
                ...
                if pred > 0 : 
                    ...
                    cv2.imwrite(f"./data/{name}/51{name}.jpg", resized)
                    Image1 = Image.open(f"./2.png")
                    # make a copy the image so that the  
                    # original image does not get affected 
                    Image1copy = Image1.copy() 
                    Image2 = Image.open(f"./data/{name}/51{name}.jpg") 
